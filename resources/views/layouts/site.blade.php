...
<head>
    ...
    @php
        ...
        $robots = request()->routeIs('projects.index') && request()->query()
...
<header class="site-header" data-header>
    <div class="lux-container header-inner">
        <a class="site-brand" href="{{ route('home') }}" aria-label="{{ $siteSettings['site_name'] ?? 'آریا سازه' }}">
            <img src="{{ asset('assets/construction/logo.svg') }}" alt="" width="168" height="52">
        </a>
        <button class="menu-toggle" type="button" aria-label="باز کردن منو" aria-expanded="false" data-menu-toggle>
            <span></span><span></span>
        </button>
...
<footer class="site-footer">
    <div class="lux-container footer-main">
        <div class="footer-brand">
            <img src="{{ asset('assets/construction/logo.svg') }}" alt="{{ $siteSettings['site_name'] ?? 'آریا سازه' }}" width="190" height="58">
            <p>{{ $siteSettings['site_tagline'] ?? 'خانه‌ها و برج‌هایی برای نسل‌های آینده.' }}</p>
        </div>
        ...
        <div class="footer-news">
            <strong>همراه آریا سازه</strong>
            <p>برای شروع پروژه یا دریافت مشاوره با ما در ارتباط باشید.</p>
            <a class="outline-btn" href="{{ route('contact') }}">شروع گفتگو</a>
        </div>
    </div>
    <div class="lux-container footer-bottom">
        <span>© {{ date('Y') }} {{ $siteSettings['site_name'] ?? config('app.name') }}</span>
        <span>ساخته شده توسط تیم فنی <a class="text-arrow" href="https://redcoweb.ir" target="_blank" rel="noopener noreferrer">redcoweb.ir</a></span>
    </div>
</footer>
